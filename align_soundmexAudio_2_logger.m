...
%                     TTLHigh = find(abs(diff(Ttl_status))>0.018)+1;
                    TTLHigh = find(abs(diff(Ttl_status))>0.032)+1;
                    if isempty(TTLHigh)
...
if sum(Pulse_idx_audio(Pulse_idx_audio(Duplicates))==Pulse_idx_audio(Duplicates))==length(Duplicates)
 ...
 % Check that there is at least 2 pulses for each file otherwise unreliable
 [UFile,IndOut,~] = unique(File_number);
 WrongFile = UFile(find(diff(IndOut)==1));
 for ff=1:length(WrongFile)
    Pulse_idx_audio(File_number==WrongFile(ff))=[];
    Pulse_samp_audio(File_number==WrongFile(ff))=[];
    File_number(File_number==WrongFile(ff))=[];
 end
