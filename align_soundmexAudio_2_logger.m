...
pnames = {'Method', 'Session_strings','TTL_pulse_generator'};
dflts  = {'risefall',{'record only start' 'record only stop'},'MOTU'};
[Method, Session_strings, TTL_pulse_generator] = internal.stats.parseArgs(pnames,dflts,varargin{:});
...
elseif strcmp(TTL_pulse_generator, 'Avisoft')
    TTL_paramfile = dir(fullfile(Audio_dir,'*unique_ttl_params.mat')); % this is the parameter file generated by generate_zero_playback_file.m (code generating TTL pulse playback sequences)
    TTL_param = load(fullfile(TTL_paramfile.folder, TTL_paramfile.name));
end
